...
namespace TestSelector.Services.TestMatcher.Matchers
{
    public abstract class BaseTestMatcher : ITestMatcher
    {
        ...
        protected IEnumerable<TestMatch> ExtractTestMatches(CodeCoverage.Model.CodeCoverage coverage, FileChange change)
        {
            ...
            //Total time would be O(min(N,M) x log(min(N,M) x max(N,M))
            foreach (var range in coverage.Ranges)
            {
                intervalTree.InsertInterval(range.To, range.From, coverage.TestId);
            }
